...
async def api_open_pr(issue_id: int):
    ...
    desc = generate_pr_description(issue_id)
    pr_body = desc["pr_body"] if desc else None

    try:
        url = await asyncio.to_thread(_do_open_pr, issue, pr_body)
        return {"pr_url": url}
    except ValueError as e:
        if str(e) == "no_code_blocks" and desc:
            return {"fallback": True, "pr_body": desc["pr_body"], "compare_url": desc["compare_url"]}
...
def _do_open_pr(issue: dict[str, Any], pr_body: str | None = None) -> str:
    ...
    files_section = re.search(r"📁 Files[\s\S]*?(?=📝 Step|💡 One|$)", action_plan)
    file_path = None
    if files_section:
    ...
    if not file_path:
        raise ValueError("no_code_blocks")
    ...
    resp = client.get(f"https://api.github.com/repos/{fork_full}/git/ref/heads/{default_branch}")
    ...
    if content_resp.status_code != 200:
        ...
        if tree_resp.status_code == 200:
            ...
            match = next(
                (e["path"] for e in tree if e["type"] == "blob" and e["path"].endswith(basename)),
                None,
            )
            if match:
                file_path = match
    ...
    if not applied_any:
        ...
        additions = [re.sub(r"\s*✅.*", "", b).strip() for b in after_blocks]
        new_content = current_content + "\n\n" + "\n\n".join(additions)
    ...
    pr_resp = client.post(
        ...
        json={
            ...
            "body": pr_body
            or (
                f"🤖 Auto-generated fix from triage report\n\n"
                f"Issue: {html_url}\n\n"
                f"This PR was automatically generated by the Issue Triage system."
            ),
    ...
    save_pr_info(issue["id"], pr_url, head_sha)
...
async def api_save_preferences(body: PreferencesBody):
    ...
    updated = {
        ...
        "show_dismissed": (body.show_dismissed if body.show_dismissed is not None else current["show_dismissed"]),
    }
